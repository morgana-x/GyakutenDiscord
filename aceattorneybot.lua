...
local activekeys = {};
local nextClearKeyFrame = 0;
local nextClearKey = false;
function ST_received(id)
	...
	while true do
		...
		if p then
			...
			--for a in string.gmatch(p, "[^%s]+") do
			local keyNum = tonumber(p) or 0;
			console:log(tostring(keyNum));
			--if (not activekeys[keyNum]) then
			emu:addKey(keyNum);
			nextClearKeyFrame =2
			nextClearKey = true;
			table.insert(activekeys,keyNum)
			--end
			--end
		else
...
function GM_Frame()
	if (nextClearKeyFrame > 0) then nextClearKeyFrame = nextClearKeyFrame-1; end
	if (not (nextClearKeyFrame == 0 and nextClearKey)) then return end
	nextClearKey = false;
